...
//referência i
BTreeNode* BTree::doSearch(BTreeNode* node, int value, int& i)
{
    i = 0;
    while (i < node->n && value > node->key[i])
        i++;

    if (i < node->n && value == node->key[i])
        return node;
    else
    {
        if (node->leaf == true)
...
void BTree::insertNonFull(BTreeNode* x, int value)
{
    int i = x->n - 1;
...
bool BTree::doRemove(BTreeNode* pt, int x)
{    
    BTreeNode* u;
    BTreeNode* w;
    BTreeNode* y;
    ...
    int i = 0;
    while (i < pt->n && x > pt->key[i])
        i++;
    
    if (i < pt->n && x == pt->key[i])
    {
        if (pt->leaf)
        {
            //Caso 1
        ...
        else
        {
            ...
            if (pt->c[i]->n > t - 1)
            {
                ...
                return doRemove(pt->c[i], u->key[u->n - 1]);
            }
            else
            {
                DISK_READ(pt->c[i + 1]);
                ...
                if (pt->c[i + 1]->n > t - 1)
                {
                    ...
                    return doRemove(pt->c[i + 1], z->key[0]);
                }
                else
                {
                    //Caso 2c
